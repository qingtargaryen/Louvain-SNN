...
class Louvain:
    ...
    def generator_communities_by_snn(self):
        ...
        for n1 in G1:
            for n2 in G1[n1]:
                ...
                if sim >= 0.6 and self._overlap(n1,n2) > 2:
                    # Use a set to avoid duplicate nodes
                    edges.append((n1,n2))
                    new_community = {n1, n2}
                    merged = False
    ...
    def execute_by_snn(self, stage_collector=None):
        """
        stage_collector: 若为 list，则依次追加 {"title": str, "partition": list[list[int]]}
        供网页绘制各阶段结构图（划分均在原始节点编号上）。
        """
        G_nodes = set(self.G1.nodes())

        def snap(title, parts):
            if stage_collector is None:
                return
            p = ensure_full_partition(parts, G_nodes)
            stage_collector.append({"title": title, "partition": p})

        snap("原始网络（未划分）", [list(G_nodes)])

        comm = self.generator_communities_by_snn()
        snap("SNN 初始社区（相似网络聚类）", comm)

        c = self.Optimized(comm)
        snap("边界节点优化后", c)

        # 对获得的初始社区进行约束，要求每个社区内的节点大于3个
        communities = [list(cc) for cc in c if len(cc) > 3]
        snap("过滤小社区后（|C|>3）", communities)

        self.origin_communities(communities)
        snap("Louvain 初始化（超级节点合并后）", self.get_communities())
        ...
        while True:
            ...
            round_idx += 1
            if mod_inc:
                if round_idx <= LOUVAIN_STAGE_ROUND_LIMIT:
                    snap(
                        "模块度局部优化 第 {} 轮".format(round_idx),
                        self.get_communities(),
                    )
                self.second_stage()
                if round_idx <= LOUVAIN_STAGE_ROUND_LIMIT:
                    snap(
                        "网络凝聚 第 {} 轮".format(round_idx),
                        self.get_communities(),
                    )
            else:
                break

        final = self.get_communities()
        snap("最终结果（收敛）", final)
        return final
...
WEB_PLOT_MAX_NODES = 100

# 各阶段过程图：Louvain 外圈最多记录轮数（每轮 2 张图：局部优化后 + 凝聚后）
LOUVAIN_STAGE_ROUND_LIMIT = 4


def ensure_full_partition(communities, all_nodes):
    """
    将社区列表补全为覆盖 all_nodes 的划分；未覆盖节点各为单点社区。
    communities: 可迭代的社区，元素为节点可迭代对象。
    """
    if all_nodes is None:
        return [list(c) for c in communities] if communities else []
    covered = set()
    out = []
    for comm in communities or []:
        s = {n for n in comm if n in all_nodes}
        if s:
            out.append(list(s))
            covered |= s
    for n in sorted(all_nodes - covered):
        out.append([n])
    return out


def showCommunity_to_png_base64(G, partition, pos, dpi=110, title=None, figsize=None):
    """
    与 showCommunity 相同的绘制逻辑，输出 PNG 的 base64（供网页嵌入）。
    不在此判断网络规模，由调用方控制。
    """
    cluster = {}
    labels = {}
    ...
    if title:
        ...
        fig.suptitle(title, fontsize=12, color="#1a1a1a", fontweight="600")
...
def run_louvain_snn_web(path, sim_thread=0.4, num_runs=20):
    """
    Web 接口用：每次运行前从磁盘重新加载图（相互独立），统计模块度与耗时。
    返回 JSON 可序列化结构。
    最后一轮运行可额外采集各阶段划分并生成 stage_plots（节点数不超过阈值时）。
    """
    ...
    obj_vis = Graph()
    G_vis = obj_vis.createGraph(path)
    n_nodes = G_vis.number_of_nodes()
    pos = None
    if n_nodes <= WEB_PLOT_MAX_NODES:
        try:
    ...
    for i in range(num_runs):
        ...
        start_time = time.time()
        algorithm = Louvain(G, G1, sim_thread)
        collector = [] if (i == num_runs - 1 and pos is not None) else None
        communities = algorithm.execute_by_snn(stage_collector=collector)
        if collector is not None:
            stage_snapshots = collector
        cost = time.time() - start_time
        zhi = round(cal_Q(communities, G2), 4)
        avg += zhi
        all_cost += cost
        runs_detail.append(
            {"run": i + 1, "q": zhi, "time_sec": round(cost, 6)}
        )
        last_communities = communities
        mp[zhi] = mp.get(zhi, 0) + 1

    sizes = sorted([len(c) for c in last_communities], reverse=True)

    plot_png_base64 = None
    plot_message = None
    stage_plots = []
    stage_plots_message = None

    if n_nodes > WEB_PLOT_MAX_NODES:
        plot_message = f"节点数为 {n_nodes}（>{WEB_PLOT_MAX_NODES}），不绘制网络结构图。"
        stage_plots_message = plot_message
    elif pos is not None:
        try:
            plot_png_base64 = showCommunity_to_png_base64(
                G_vis, last_communities, pos
            )
        except Exception:
            plot_message = "布局或绘图失败，未生成网络图。"
        for ent in stage_snapshots:
            try:
                b64 = showCommunity_to_png_base64(
                    G_vis,
                    ent["partition"],
                    pos,
                    title=ent["title"],
                ...
                continue
        if not stage_plots and stage_snapshots:
            stage_plots_message = "各阶段图绘制失败。"
    else:
        stage_plots_message = "无法生成布局，未绘制阶段图。"

    return {
        "avg_q": round(avg / num_runs, 4),
        "avg_time_sec": round(all_cost / num_runs, 6),
        "q_distribution": [
            {"q": k, "count": v} for k, v in sorted(mp.items())
        ],
        "runs": runs_detail,
        "last_communities": [sorted(list(c)) for c in last_communities],
        "num_communities": len(last_communities),
        "community_sizes": sizes,
        "num_nodes": n_nodes,
        "plot_png_base64": plot_png_base64,
        "plot_message": plot_message,
        "stage_plots": stage_plots,
        "stage_plots_message": stage_plots_message,
    }
