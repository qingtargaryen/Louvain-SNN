...
class Louvain:
    ...
    def generator_communities_by_snn(self):
        ...
        motif_graph = nx.from_edgelist(edges)
        component_count = len(self._G.keys()) // 50 + 1
        components = [c for c in nx.connected_components(motif_graph)]
        components = [[len(c), c] for c in components]
        components.sort(key=lambda x: x[0], reverse=True)
        important_components = [components[comp][1] for comp
                                in range(component_count if len(components)>=component_count else len(components))]
        self.blocks = [list(graph) for graph in important_components]
        new_edges = [(n_1, n_2) for nodes in self.blocks for n_1 in nodes for n_2 in nodes if n_1!= n_2]
