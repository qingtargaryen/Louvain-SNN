        ...
        for n1 in G1:
            for n2 in G1[n1]:
                if n1 >= n2:
                    continue
                # sim = cal_snn_sim_between_two_nodes(self._G, n1, n2)
                if cal_snn_sim_between_two_nodes(G1, n1, n2) >= self.SIM_Thread:
                    # Use a set to avoid duplicate nodes
                    new_community = {n1, n2}
                    merged = False
...
def get_mean_time_max_q_by_SNN(path, op, SIM_Thread):
    """
    ...
    if op:
        edm = edmot.EdMot(G1,ccnt // 50 + 1,cutoff,0.3)
        edmot_G1 = edm.fit()
    ...
    while iter_time < 100:
        ...
        if op:
            algorithm = Louvain(G, edmot_G1, SIM_Thread)
        else:
...
if __name__ == '__main__':
    ...
    get_mean_time_max_q_by_SNN(path, True, SIM_Thread)  # louvain_SNN的迭代过程
    ...
    #             #     continue
    #             # sim = cal_snn_sim_between_two_nodes(self._G, n1, n2)
    #             snn = cal_snn_sim_between_two_nodes(self.G1, n1, n2)
    #             if snn > mx and snn >= SIM_Thread:
    #                 mx = snn
    #                 ans = n2
    #         if mx != 0:        
    #             new_community = {n1, ans}
    #             # edges.append((n1,ans))
    #             merged = False
    #             # Check if the new community intersects with any existing community
    #             for i, community in enumerate(communities):
    ...
    #     return final_communities
        ...
        # execute
        # communities = [[15,18,21,24,29,45,51],[14,16,33,37,38,40,43,50,52]]
        # a = [15,18,21,24,29,45,51]
        # b = [14,16,33,37,38,40,43,50,52]
        # for i in range(1,62):
